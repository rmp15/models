...
parameters <- list(alpha_m=rep(0.,M),beta_m=rep(0.,M),beta_0=0.,log_mu=matrix(0.,N,T), pi=matrix(0.,N,T) ,log_prec_rw=0.,log_prec_epsilon=0.,log_prec_int_m=0.,log_prec_slp_m=0.)
...
alpha_0 <- extract.variable(sd,'alpha_0','fixed')
...
library(INLA)
dat$t <- rep(seq(1,T),N)
dat$month2 <- dat$month
dat$t3 <- dat$t2 <- dat$t
dat$e <- seq(1:nrow(dat))
fml <- deaths ~ 1 + 
	f(month, model='rw1') + 
	f(t, model = "linear", mean.linear = 0, prec.linear = 1e-2) +
	f(month2,t2,model='rw1',hyper = list(theta = list(prior = "loggamma", param = c(1 , 1e-3))))+ 
	f(t3,model='rw1',hyper = list(theta = list(prior = "loggamma", param = c(1 , 1e-3))))+ 
...
plot.inla.rw1 <- inla.fit$summary.random$t3
